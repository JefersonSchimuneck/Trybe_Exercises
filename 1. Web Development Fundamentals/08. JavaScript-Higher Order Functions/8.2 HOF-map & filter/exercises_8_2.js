...
function nameAndAge() {
  const ageReleasedOrder = books.sort(
    (bookA, bookB) =>
      bookA.releaseYear -
      bookA.author.birthYear -
      (bookB.releaseYear - bookB.author.birthYear)
  );
...
return booksB.find(book => book.author.name.split('. ').length === 4).name;
}
